...
from app.services.security_event_ingestion import SecurityEventIngestionResult
...
class SecurityEventService:
    ...
    def __init__(self, db: Session, detection_engine: DetectionEngine | None = None):
        ...
        self.incident_correlation_service = IncidentCorrelationService(db)
        if detection_engine is not None:
            self.detection_engine = detection_engine
        else:
            rule_provider = DetectionRuleProvider(DetectionRuleService(db))
            self.detection_engine = DetectionEngine(rule_provider=rule_provider)

    def create_event(self, *, event: SecurityEventCreate) -> SecurityEvent:
    ...
    def create_event_with_detection(self, *, event: SecurityEventCreate) -> SecurityEventIngestionResult:
        """
        Create an event, matches, alerts, and their incident associations.

        The method commits nothing, preserving the existing caller-owned
        transaction: all records succeed or roll back together.
        """
        created_event = self.create_event(event=event)
        detection_result = self.detection_engine.evaluate_event(event=created_event)
        ...
        for rule in detection_result.matched_rules:
            ...
            alert = self.alert_service.create_alert(
                ...
                description=rule.description or created_event.message,
                opened_at=created_event.event_timestamp,
            )
            self.incident_correlation_service.correlate_alert(alert=alert)

        return SecurityEventIngestionResult(event=created_event, detection_result=detection_result)
